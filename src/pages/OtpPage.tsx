...
export default function OtpPage() {
  ...
  const canSubmit = otp.trim().length > 0 && submittingStep !== 'otp'
  ...
  return (
    ...
    >
      <div className="space-y-8">
        ...
        <FormField label="OTP">
          <div className="flex items-start gap-3 rounded-3xl border border-slate-200 bg-white px-4 py-3 shadow-sm transition focus-within:border-sky-400 focus-within:ring-4 focus-within:ring-sky-100 dark:border-white/10 dark:bg-white/5 dark:focus-within:ring-sky-500/15">
            <MessageSquareText className="mt-1 h-5 w-5 shrink-0 text-slate-400" />
            <textarea
              value={otp}
              onChange={(event) => setOtp(event.target.value)}
              className="min-h-[80px] w-full resize-none bg-transparent text-base text-slate-900 outline-none placeholder:text-slate-400 dark:text-white"
